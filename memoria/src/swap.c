...
void inicializar_swap(){
	...
	crear_swap(configuracion->TAMANIO_SWAP);
}

// crear archivo --> devuelve el file descriptor
int crear_swap(uint32_t tamanio_en_bytes){
	char* path_to_file = string_from_format("%s/swap",configuracion->PATH_SWAP);

	if (access(path_to_file, F_OK) == 0) // si por alguna razon ya existe el archivo, borralo
		remove(path_to_file);
	fd = open(path_to_file, O_RDWR|O_TRUNC|O_CREAT|O_EXCL,S_IROTH|S_IWOTH);
	if (fd == -1){
